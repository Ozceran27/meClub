...
const EventoPartidosModel = {
  listarPorEvento: async (eventoId) => {
    const [rows] = await db.query(
      `SELECT evento_partido_id, evento_id, sede_id, equipo_local_id, equipo_visitante_id,
              fecha, marcador_local, marcador_visitante, estado, creado_en, actualizado_en
  ...
  crear: async (eventoId, payload) => {
    const [result] = await db.query(
      `INSERT INTO evento_partidos
       (evento_id, sede_id, equipo_local_id, equipo_visitante_id, fecha, marcador_local, marcador_visitante, estado)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        eventoId,
        payload.sede_id,
