...
export const extractNightRange = (club) => {
  ...
  const startCandidates = [
    ...
    ...configs.map((cfg) => cfg && (cfg.inicio ?? cfg.desde ?? cfg.start ?? cfg.startTime)),
  ];

  const endCandidates = [
    ...NIGHT_END_KEYS.map((key) => club?.[key]),
    ...configs.map((cfg) => cfg && (cfg.fin ?? cfg.hasta ?? cfg.end ?? cfg.endTime)),
  ];
...
export const selectHourlyPrice = ({ cancha = {}, club = {}, horaInicio, horaFin, tarifa } = {}) => {
  const tarifaPrecio = toNumberOrNull(tarifa?.precio);
...
export const determineRateType = ({ horaInicio, club } = {}) => {
  const nightRange = extractNightRange(club);
